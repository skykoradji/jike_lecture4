/**
 * https://github.com/dialogflow/dialogflow-nodejs-client-v2/blob/master/samples/structjson.js
...
function jsonValueToProto(value) {
  ...
  if (value === null) {
    ...
    console.warn({ value, type: typeof value }, 'Unsupported value type');
  }
  return valueProto;
}
...
function valueProtoToJson(proto) {
  ...
  if (JSON_SIMPLE_VALUE_KINDS.has(proto.kind)) {
    ...
    if (!proto.listValue || !proto.listValue.values) {
      console.warn(proto, 'Invalid JSON list value proto: ');
    }
    ...
    console.warn(proto, 'Unsupported JSON value proto kind: ');
    return null;
  }
}
